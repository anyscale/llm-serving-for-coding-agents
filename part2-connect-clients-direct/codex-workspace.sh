...
# Feature disables keep Codex to plain function tools + the local MCP: its built-in web_search /
# image generation / plugins would hit hosted routes the custom provider doesn't serve, so we turn
# them off and get web search from the local Brave MCP in .codex/config.toml instead. The -c flags
# add the provider inline (requires_openai_auth=false, so a clean ~/.codex needs no OpenAI login);
