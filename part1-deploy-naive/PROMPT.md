...
Also required:
- **Load weights from S3, not Hugging Face**: `model_source="s3://llm-guide/data/ray-serve-llm/hf_repo/Qwen3.6-27B-FP8/"` — Ray Serve LLM auto-downloads a remote `s3://` source to local (stock image, no RunAI streamer; cluster needs S3 read access).
