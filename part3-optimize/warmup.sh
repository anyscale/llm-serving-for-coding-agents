...
# g7e node provisioning (minutes) + ~85 s HF weight load + ~75 s cold compile (or ~9 s with compile
# cache once rebuilt for vLLM 0.25.1), so the default budget is 15 minutes (override with WARMUP_TIMEOUT_S).
#
